...
%   Extract Ident in this function?)
for i = 1:length(eq_chars)
    ...
    if(chars(X_orig(idx_matched,end)).char(1) == '\')
        printChar = strcat('\',chars(X_orig(idx_matched,end)).char);
    else
       printChar = chars(X_orig(idx_matched,end)).char;
    end
